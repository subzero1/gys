...
<input type="hidden" name="dispatchStr" value="/system/flwh/fzcsg.jsp">
...
<div align="left">
	<div style="width:600">
	<br/>
		<div id="tree-title" align="center">高原地区施工调整系数</div>
	</div>
	
	<table style="width:600" id="tree-table">
		<%
		queryBuilder = new HibernateQueryBuilder(Ga14_b3jcfl.class);
		queryBuilder.eq("fylb",new Integer(1));
		queryBuilder.eq("bz",new Integer(1));
		...
		while(ro.next()){
			...
			<tr>
				...
				<td>机械调整系数</td>
				<td width="80"><input type="text" name="JXFL" size="4" value="<%=NumberFormatUtil.roundToString(ga14.getJxfl())%>"></td>
			</tr>
			<%
		}
		%>
	...
	<div style="width:600">
	<br/>
		<div id="tree-title" align="center">其他地区调整系数</div>
	</div>
	...
	<table style="width:600" id="tree-table">
		...
		queryBuilder = new HibernateQueryBuilder(Ga14_b3jcfl.class);
		queryBuilder.eq("fylb",new Integer(1));
		queryBuilder.eq("bz",new Integer(2));
		queryBuilder.eq("flk_id",flk_id);
		ro = queryService.search(queryBuilder);
		while(ro.next()){
			ga14 = (Ga14_b3jcfl)ro.get(Ga14_b3jcfl.class.getName());
